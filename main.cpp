#include <iostream>

using namespace std;

// Вывод массива
void print_array(string name, int array[], int N)
{
    cout << name << ": ";
    for (int i = 0; i < N; i++)
        cout << array[i] << " ";
    cout << endl;
}


int main()
{
    srand(time(0));

    // Размер массива
    const int N = 10;

    // Инициализация 3 массивов
    int array_1[N];
    int array_2[N];
    int array_3[N];

    // Заполнение случайными числами 2 массива
    for (int i = 0; i < N; i++)
    {
        // Случайные числа от 0 до 9
        array_1[i] = rand() % 10;
        array_2[i] = rand() % 10;
    }

    // Указатели на массивы
    int* ptr_a1 = array_1;
    int* ptr_a2 = array_2;
    int* ptr_a3 = array_3;

    asm(
            "mov %[ptr_a1], %%rsi   \n\t" // Ссылка на первый массив
            "mov %[ptr_a2], %%rdi   \n\t" // Ссылка на второй массив
            "mov %[ptr_a3], %%rdx   \n\t" // Ссылка на третий массив
            "movl %[N], %%ecx       \n\t" // Размерность массива
            
        "main_loop:                 \n\t"
            "movl (%%rsi), %%eax    \n\t" // Берем значение из первого массива
            "movl (%%rdi), %%ebx    \n\t" // Берем значение из второго массива
            "cmpl %%eax, %%ebx      \n\t" // Сравниваем эти значение
            "jg write               \n\t" // Если значение второго больше первого, то пропускаем действие 
            "movl %%ebx, %%eax      \n\t" // Иначе выбираем для записи значение из второго массива
        
        "write:                     \n\t" 
            "movl %%eax, (%%rdx)    \n\t" // Помещаем выбранное число в 3 массив
            "add $4, %%rsi          \n\t" // Переходим к след. елементу у всех массивов
            "add $4, %%rdi          \n\t" // (4 из-за того, что int = 4 байта)
            "add $4, %%rdx          \n\t"
            "loop main_loop         \n\t" // Повторяем действие N раз
            :
            // Переменные, которые использовались во вставке
            :[ptr_a1]"m"(ptr_a1), [ptr_a2]"m"(ptr_a2), [ptr_a3]"m"(ptr_a3), [N]"m"(N)
            // Регистры, которые использовались во вставке
            :"%rax", "%rbx", "%rcx", "%rdx", "%rsi", "%rdi"
       );

    // Вывод первых 2 массивов
    print_array("Array 1", array_1, N);
    print_array("Array 2", array_2, N);
    
    // Вывод линии плюсов (для лучшей визуализации)
    for (int i = 0; i < 35; i++)
        cout << "-";
    cout << endl;

    // Вывод итогового массива
    print_array("Array 3", array_3, N);

    return 0;
}
